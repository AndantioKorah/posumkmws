...
<div class="row p-3">
    <div class="col-lg-12">
        <h5>Nama Item: <strong><?=$result['nama_menu_merchant']?></strong></h5>
        <h5>Stock Akhir: <strong><?=formatCurrencyWithoutRp($result['transaksi']['total'])?></strong></h5>
    </div>
    <div class="col-lg-12 table-responsive">
        <table class="table table-hover table-striped" id="table_stock_opname_detail">
            ...
            <tbody>
                <?php if($result['transaksi']['list']){ $no = 1; foreach($result['transaksi']['list'] as $rs){
                    ?>
                    <tr>
...
<script>
    $(function(){
        $('.nama_detail_stock_opname').html('<?=$result['nama_menu_merchant']?>')
    })
</script>