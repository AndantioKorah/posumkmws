...
<div class="row">
    <div class="col-lg-12 table-responsive">
        <table class="table table-hover table-striped" id="table_stock_opname">
            ...
            <tbody>
                <?php if($result && $result['menu']){ $no = 1; foreach($result['menu'] as $m){ 
                    // dd(json_encode($result['menu'])); 
                    ?>
                    <tr onclick="openDetailStockOpnameMenu('<?=($m['id'])?>')" href="#modal_stock_opname" data-toggle="modal" class="tr_menu_merchant">
...
<script>
    ...
    function openDetailStockOpnameMenu(id){
        $('#modal_stock_opname_content').html('')
        $('#modal_stock_opname_content').append(divLoaderNavy)
        $('#modal_stock_opname_content').load('<?=base_url('laporan/C_Laporan/openDetailStockOpnameMenu/')?>'+id, function(){
            $('#loader').hide()
        })
    }
</script>